...
// peopleIdMap is an object. Using “square bracket notation” ==>> when key = "multiword"/"number"/"have space"/"special characters"
...
function lastNameFilter(namesArray) {
    const lastNameArray = [];

    namesArray.forEach((name) => {
        const splitName = name.split(" ");
        lastNameArray.push(splitName[splitName.length - 1]);
    });

    return lastNameArray;
}

// console.log("Everyone last name:\n", lastNameFilter(names));

//////// CHALLENGE: Filter to the people who followed the right
// "right format" is "<first name> <last name>" with a single space!

function rightNameFormat() {
    const rightFormat = /^\w+ \w+$/;

    let matchFormatRule = [];

    names.forEach((name, index) => {
        if (name.match(rightFormat)) {
            matchFormatRule.push(name);
        }
    });

    return matchFormatRule;
}

// console.log("good students", rightNameFormat());

//////// CHALLENGE: Change everyone's name to "Title Case"
function titleCase(namesArray) {
    const titledNames = namesArray.map((name) => {
        const splitName = name.split(" ");

        splitName.forEach((value, index, arr) => {
...
function removeLastNameZ(value) {
    const filterArray = value.filter((currentValue, index, arr) => {
...
const result = removeLastNameZ(titleCase(rightNameFormat()));

result.forEach((name, index, arr) => {
    arr[index] = `Hi ${name} ! Do you want to a favor for me!`;
});

// console.log(titleCase(rightNameFormat()));
console.log("result: \n", result);
